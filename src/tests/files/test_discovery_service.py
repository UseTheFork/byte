...
@pytest.mark.asyncio
async def test_discovers_git_repo_files(application: Application):
...
@pytest.mark.asyncio
async def test_respects_gitignore_patterns(application: Application):
    """Test that discovery service respects .gitignore patterns."""
    from byte.files import FileDiscoveryService

    # Create a .pyc file that should be ignored
    await create_test_file(application, "test.pyc", "compiled python")
...
@pytest.mark.asyncio
async def test_find_files_with_pattern(application: Application):
    """Test that discovery service can find files matching a pattern."""
    from byte.files import FileDiscoveryService

    # Create some test files
    await create_test_file(application, "test_module.py", "# test module")
    await create_test_file(application, "another_test.py", "# another test")
...
@pytest.mark.asyncio
async def test_get_relative_paths(application: Application):
...
@pytest.mark.asyncio
async def test_add_and_remove_file(application: Application):
    ...
    # Create a new file
    new_file = await create_test_file(application, "dynamic_file.txt", "dynamic content")
...
@pytest.mark.asyncio
async def test_respects_custom_ignore_patterns(application: Application):
    ...
    custom_dir.mkdir()
    await create_test_file(application, "custom_ignored_dir/should_be_ignored.txt", "ignored content")
...
@pytest.mark.asyncio
async def test_filter_files_by_extension(application: Application):
    """Test that discovery service can filter files by extension."""
    from byte.files import FileDiscoveryService

    # Create files with different extensions
    await create_test_file(application, "script.py", "# python script")
    await create_test_file(application, "app.js", "// javascript app")
    await create_test_file(application, "notes.txt", "text notes")
...
@pytest.mark.asyncio
async def test_discovers_files_in_nested_directories(application: Application):
    ...
    await create_test_file(application, "src/main.py", "# main module")
    await create_test_file(application, "src/utils/helpers.py", "# helper functions")
...
@pytest.mark.asyncio
async def test_find_files_with_fuzzy_matching(application: Application):
    ...
    await create_test_file(application, "src/test_utils.py", "# test utilities")

    # Pattern in directory path (should score lower)
    test_dir = application.base_path("tests")
    test_dir.mkdir()
    await create_test_file(application, "tests/helpers.py", "# helper functions")

    # Pattern at start of filename (exact match, highest priority)
    await create_test_file(application, "test.py", "# test module")
...
@pytest.mark.asyncio
async def test_find_files_case_insensitivity(application: Application):
    """Test that pattern matching is case-insensitive."""
    from byte.files import FileDiscoveryService

    # Create files with mixed case names
    await create_test_file(application, "TestModule.py", "# test module")
    await create_test_file(application, "testhelper.py", "# test helper")
    await create_test_file(application, "MyTest.py", "# my test")
...
@pytest.mark.asyncio
async def test_ignores_files_in_ignored_parent_directory(application: Application):
    ...
    await create_test_file(application, "__pycache__/module.pyc", "compiled")
    await create_test_file(application, "__pycache__/subdir/nested.pyc", "compiled nested")
    await create_test_file(application, "__pycache__/subdir/source.py", "# source in pycache")
...
@pytest.mark.asyncio
async def test_refresh_clears_and_rebuilds_cache(application: Application):
    """Test that refresh actually clears old entries and rebuilds from scratch."""
    from byte.files import FileDiscoveryService

    # Create initial file
    initial_file = await create_test_file(application, "initial.py", "# initial file")
    ...
    initial_file.unlink()
    await create_test_file(application, "new.py", "# new file")
...
@pytest.mark.asyncio
async def test_add_file_rejects_ignored_files(application: Application):
    ...
    # Create a .pyc file that should be ignored
    pyc_file = await create_test_file(application, "ignored.pyc", "compiled python")
...
@pytest.mark.asyncio
async def test_remove_file_for_nonexistent_file(application: Application):
...
@pytest.mark.asyncio
async def test_handles_unreadable_files(application: Application):
...
@pytest.mark.asyncio
async def test_handles_binary_files(application: Application):
