...
class TestEditFormatHistoricMessages:
    ...
    async def test_replace_blocks_in_historic_messages_hook(
        ...
        ai_message_indices = [
            i for i, msg in enumerate(original_messages) if isinstance(msg, AIMessage)
        ]

        # Store original content of messages that should be modified
        messages_to_modify = (
            ai_message_indices[:-2] if len(ai_message_indices) > 2 else []
        )

        # Store original content of last 2 AIMessages (should not change)
        last_two_indices = (
            ai_message_indices[-2:]
            if len(ai_message_indices) > 2
            else ai_message_indices
        )
        last_two_original_content = {
            idx: str(original_messages[idx].content) for idx in last_two_indices
        }

        # Execute the hook
        result_payload = (
            await edit_format_service.replace_blocks_in_historic_messages_hook(payload)
        )
        ...
        for idx in last_two_indices:
            assert (
                str(modified_messages[idx].content) == last_two_original_content[idx]
        ...
        for idx in messages_to_modify:
            ...
            assert (
                ...
                or "Changes applied" in modified_content
