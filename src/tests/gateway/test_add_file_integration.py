"""Integration tests for Requests.AddFile over real WebSocket."""
...
@pytest.mark.asyncio
async def test_add_file_success_over_ws(gateway: GatewayService, config: Any) -> None:
    """Test add_file request returns success response over real WebSocket."""
...
@pytest.mark.asyncio
async def test_add_file_failure_over_ws(gateway: GatewayService, config: Any) -> None:
    """Test add_file request returns error response when file not found."""
    ws = await _connect_and_auth(gateway, config)
    try:
        # Send add_file request with non-existent file
        add_file_request = RpcRequest(
            jsonrpc="2.0",
            method="add_file",
        ...
        assert "Failed to add" in response.get("error", {}).get("message", "")
...
@pytest.mark.asyncio
async def test_add_file_missing_param_over_ws(gateway: GatewayService, config: Any) -> None:
    """Test add_file request with missing file_path parameter."""
    ws = await _connect_and_auth(gateway, config)
    try:
        # Send add_file request without file_path
        add_file_request = RpcRequest(
            jsonrpc="2.0",
            method="add_file",
            params={},
...
@pytest.mark.asyncio
async def test_add_file_wildcard_pattern_over_ws(gateway: GatewayService, config: Any) -> None:
    """Test add_file request with wildcard pattern."""
    ws = await _connect_and_auth(gateway, config)
    try:
        # Send add_file request with wildcard pattern
        add_file_request = RpcRequest(
            jsonrpc="2.0",
            method="add_file",
            params={"file_path": "*.md"},
