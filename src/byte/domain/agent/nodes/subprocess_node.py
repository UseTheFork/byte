...
class SubprocessNode(Node, UserInteractive):
	...
	async def __call__(self, state: BaseState, config: RunnableConfig):
		...
		if should_add:
			...
			result_message = f"""<subprocess_result>
<command>{subprocess_result.command}</command>
<exit_code>{subprocess_result.exit_code}</exit_code>
<stdout>
...
{subprocess_result.stderr}
			...
			return Command(goto="end_node", update={"messages": [("user", result_message)]})
