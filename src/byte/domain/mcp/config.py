...
class MCPConnection(BaseModel):
    ...
    url: Optional[str] = Field(
        default=None, description="URL for HTTP transport connection"
    )
    headers: Optional[Dict[str, str]] = Field(
        default=None, description="HTTP headers for the connection"
    )
...
class AgentToolFilter(BaseModel):
    ...
    exclude: Optional[List[str]] = Field(
        default=None, description="Tools to exclude from this agent"
    )
