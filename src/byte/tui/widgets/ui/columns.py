...
# Credits: https://github.com/batrachianai/toad/blob/main/src/toad/visuals/columns.py
@rich.repr.auto
class Row(Visual):
...
class Columns:
    ...
    def get_optimal_width(self) -> int:
        ...
        optimal_width = max(sum(content.cell_length for content in row) + gutter_width for row in self.rows)
    ...
    def add_row(self, *cells: Content | str) -> Row:
        ...
        new_cells = [cell if isinstance(cell, Content) else Content(cell) for cell in cells]
    ...
    def render(self, row_index: int, render_width: int, style: Style = Style.null()) -> list[Strip]:
