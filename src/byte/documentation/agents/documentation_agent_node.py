...
class DocumentationAgentNode(BaseAgentNode):
    ...
    def get_system_template(self):
        return [
            Leaves.Preamble(role="Act as an expert in project documentation tooling and practices."),
            Leaves.Constitution(),
            Leaves.CommunicationStyle(
                rich_markdown=False,
                extra_styles=[
                    "Conciseness is about **text only**: always fully implement the requested documentation structure, pages, and configuration even if that requires many tool calls.",
                    "Prefer clear, well-structured prose — avoid jargon unless the target audience expects it",
                    "Never send acknowledgement-only responses; after receiving new context or instructions, immediately continue the task or state the concrete next action you will take.",
                ],
            ),
            Leaves.WorkflowConstraints(
                [
                    "Analyze the user's documentation request and the provided file context",
                    "If the request is ambiguous, ask clarifying questions",
                    "Identify which documentation files, configs, or navigation structures need to be modified, created, or deleted",
                    "Break down the changes into clear, sequential steps",
                    "Follow the project's configured documentation framework conventions",
                    "Keep the plan concise and actionable",
                ]
            ),
            Leaves.OperatingPrinciples(),
            Leaves.DocumentationGuidelines(),
        ]
