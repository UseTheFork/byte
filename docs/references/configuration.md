...
| `commands` | `array[LintCommandConfig]` | `[]` | List of lint commands to run on files with their target extensions |

## Lint > LintCommand

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `command` | `array[string]` | - | Command and arguments to execute for linting (e.g., ['ruff', 'check', '--fix']). Use {files} placeholder to specify where the file paths should be inserted, otherwise they will be appended to the end. |
